...
app.controller('calcController',function(){
    ...
    this.acceptClickedKey = function(clickedInput){
...
function make_parameter(the_input_array){ //called whenever "=" is pressed
    ...
    for(var i = 0; i < the_input_array.length; i++){
        if(the_input_array[i].value == "X" || the_input_array[i].value == "/"){
            inner_answer = doMath(parseFloat(the_input_array[i-1].value),parseFloat(the_input_array[i+1].value),the_input_array[i].value);
            var b = new Object();
    ...
    for(var j = 0; j < the_input_array.length; j++){
        if(the_input_array[j].value == "+" || the_input_array[j].value == "-"){
            inner_answer = doMath(parseFloat(the_input_array[j-1].value),parseFloat(the_input_array[j+1].value),the_input_array[j].value);
            var b = new Object();
            b.value = inner_answer;
            b.type = "num_key";
